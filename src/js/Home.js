...
export default class RegisterUserForm extends Component {
	...
	deletePlayer = async (event, player) => {
		const cookies = new Cookies();

		await fetch('https://players-api.developer.alchemy.codes/api/players/' + player.id, {
			method: 'DELETE',
			headers: {
				Authorization: 'Bearer ' + cookies.get('pingPongJWT'),
			},
		...
		const resp = await fetch('https://players-api.developer.alchemy.codes/api/players', {
			method: 'GET',
			headers: {
				Authorization: 'Bearer ' + cookies.get('pingPongJWT'),
			},
			...
			return (
				<tr key={player.id}>
					...
					<td className="text-right">
						<a className="delete" onClick={e => this.deletePlayer(e, player)}>
			...
			<div>
				...
				<div className="form">
					<form onSubmit={this.handleSubmit}>
						...
						<Button id="register" block bsSize="large" disabled={!this.validateForm()} type="submit">
							Register
						</Button>
					</form>
				</div>
		...
		return (
			<div className="players">
				...
				<Table striped bordered condensed hover>
					...
					<tbody>{!this.state.isLoading && this.renderPlayersList(this.state.players)}</tbody>
				</Table>
			</div>
		);
	}
	...
	render() {
		return <div className="Home">{this.props.isAuthenticated ? this.renderPlayers() : this.renderLander()}</div>;
	}
}
